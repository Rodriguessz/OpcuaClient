 ...
 */
const preregisterAbortListeners = new WeakMap();
/**
...
class OPCUAServerEndPoint extends node_events_1.EventEmitter {
    ...
    constructor(options) {
        super();
        (0, node_opcua_assert_1.assert)(!Object.hasOwn(options, "certificate"), "expecting a certificateChain instead");
        (0, node_opcua_assert_1.assert)(Object.hasOwn(options, "certificateChain"), "expecting a certificateChain");
        (0, node_opcua_assert_1.assert)(Object.hasOwn(options, "privateKey"));
        this.certificateManager = options.certificateManager;
        options.port = options.port || 0;
        this.port = parseInt(options.port.toString(), 10);
        this.host = options.host;
        (0, node_opcua_assert_1.assert)(typeof this.port === "number");
        this.#certProvider = new node_opcua_common_1.StaticCertificateChainProvider(options.certificateChain, options.privateKey);
        this._channels = {};
    ...
    dispose() {
        this.#certProvider = new node_opcua_common_1.StaticCertificateChainProvider(emptyCertificateChain, emptyPrivateKey);
    ...
    toString() {
        const txt = " end point" +
            ...
            " " +
            (0, web_1.makeSHA1Thumbprint)(this.getCertificate()).toString("hex");
        return txt;
    }
    toJSON() {
        return {
     ...
     */
    setCertificateProvider(provider) {
     ...
     */
    createReverseConnection(clientEndpointUrl, options, callback) {
        if (!this._started) {
        ...
        let ep;
        try {
            ep = (0, node_opcua_transport_1.parseEndpointUrl)(clientEndpointUrl);
        }
        catch (err) {
        ...
        const socket = node_net_1.default.createConnection({ host: hostname, port, timeout: connectTimeout }, () => {
        ...
        const onConnect = () => {
            ...
            this._preregisterChannel(channel);
            channel.initReverse(socket, { serverUri: options.serverUri, endpointUrl: options.endpointUrl }, (err) => {
                this._un_pre_registerChannel(channel);
                debugLog(chalk_1.default.yellow.bold("reverse Channel#initReverse done"), err);
                if (err) {
                    if (settled)
                        return;
                    settled = true;
    ...
    _preregisterChannel(channel) {
        ...
        (0, node_opcua_assert_1.assert)(!Object.hasOwn(this._channels, channel.hashKey), " channel already preregistered!");
    ...
    _unregisterChannel(channel) {
        ...
        (0, node_opcua_assert_1.assert)(Object.hasOwn(this._channels, channel.hashKey), "channel is not registered");
        /**
         * @event closeChannel
         * @param channel
         */
